...
class IFallingBlock(object):
    ...
    def hitGround(self): # gibt True zurueck, wenn es den "Boden" bzw ein Hindernis beruehrt, sonst False
        if (self.rotatingPosition == 0): #check part1, 2, 3 und 4
            ...
            d = self.Field.matrix[self.currPos4[0]][self.currPos4[1] + 1]
            if (a or b or c or d):
                return True
            else:
                return False
            ...
            if(self.currPos4[1] + 1 > 18):
                return True
            a = self.Field.matrix[self.currPos4[0]][self.currPos4[1] + 1]
            return a
